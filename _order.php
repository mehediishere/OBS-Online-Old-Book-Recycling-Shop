<?php 
    ...
    {
        $updateOrderSQL = "UPDATE tb_order SET `status`='$_POST[orderStatus]' WHERE `invoice`='$_POST[invoice]' ORDER BY srl DESC";
    ...
    if(isset($_GET['order']))
    {
        if($_GET['order']=="all"){
            $ord = "All";
            $orderSql = "SELECT * FROM tb_order";
        }elseif($_GET['order']=="processing"){
            $ord = "Processing";
            $orderSql = "SELECT * FROM tb_order WHERE `status`='processing'";
        }
        elseif($_GET['order']=="confirmed"){
            $ord = "Confirmed";
            $orderSql = "SELECT * FROM tb_order WHERE `status`='confirmed'";
        }
        elseif($_GET['order']=="completed"){
            $ord = "Completed";
            $orderSql = "SELECT * FROM tb_order WHERE `status`='completed'";
        }
        elseif($_GET['order']=="canceled"){
            $ord = "Canceled";
            $orderSql = "SELECT * FROM tb_order WHERE `status`='canceled'";
        }else{
            $orderSql = "SELECT * FROM tb_order";
        }
    }else{
        $orderSql = "SELECT * FROM tb_order ORDER BY srl DESC";
    }

    if(isset($_POST['src_btn']))
    {
        $var = $_POST['src_text'];
        $orderSql = "SELECT * FROM tb_order where invoice LIKE '%$var%' OR useremail LIKE '%$var%' OR status LIKE '%$var%' OR bookid LIKE '%$var%' OR bookname LIKE '%$var%' OR price LIKE '%$var%' OR totalprice LIKE '%$var%'";
    }

    if(isset($_POST['orderupdate']))
    {
        $updateOrderSQL = "UPDATE tb_order SET `status`='$_POST[orderStatus]' WHERE `invoice`='$_POST[invoice]'";
        mysqli_query($conn, $updateOrderSQL);
    }
?>
<!DOCTYPE html>
...
<body>
    ...
    <?php
        ...
        if(isset($_POST['create_invoice']) && $_POST['invoice']!=NULL)
        {
            ...
            $orderuser = mysqli_fetch_assoc(mysqli_query($conn,"SELECT `useremail` FROM `tb_order` WHERE `invoice` = '$invoice'"));
            ...
            <div style="min-width: 600px">
                ...
                <main>
                    ...
                    <table border="0" cellspacing="0" cellpadding="0">
                        ...
                        <tbody>
                            <?php
                                $invoiceSql = "SELECT * FROM tb_order WHERE `invoice`='$invoice'";
